...
public class ClientUserExtServiceImpl extends ServiceImpl<ClientUserExtMapper, ClientUserExt> implements ClientUserExtService {
    ...
    @Override
    public ClientUserExt getUserExtInfo(String userId) {
        ...
        if (ObjectUtil.isEmpty(clientUserExt)) {
            // 如果扩展信息不存在，创建一个默认的扩展信息
            clientUserExt = new ClientUserExt();
            clientUserExt.setUserId(userId);
            clientUserExt.setServiceDuration(0);
            clientUserExt.setServiceTimes(0);
            clientUserExt.setVolunteerIntegral(0);
        } else {
            // 确保数值字段不为null
            if (clientUserExt.getServiceDuration() == null) {
                clientUserExt.setServiceDuration(0);
            }
            if (clientUserExt.getServiceTimes() == null) {
                clientUserExt.setServiceTimes(0);
            }
            if (clientUserExt.getVolunteerIntegral() == null) {
                clientUserExt.setVolunteerIntegral(0);
            }
        }
