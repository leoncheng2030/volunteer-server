...
import vip.xiaonuo.auth.core.pojo.SaBaseClientLoginUserExt;
...
public class ClientUserServiceImpl extends ServiceImpl<ClientUserMapper, ClientUser> implements ClientUserService {
    ...
    public ClientLoginUser getUserById(String id) {
        ...
        if(ObjectUtil.isNotEmpty(clientUser)) {
            ...
            // 还要获取用户扩展信息
            ClientUserExt clientUserExt = clientUserExtService.getUserExtInfo(clientUser.getId());
            // 转换为SaBaseClientLoginUserExt类型
            SaBaseClientLoginUserExt extInfo = new SaBaseClientLoginUserExt();
            if (clientUserExt != null) {
                extInfo.setServiceDuration(clientUserExt.getServiceDuration());
                extInfo.setServiceTimes(clientUserExt.getServiceTimes());
                extInfo.setVolunteerIntegral(clientUserExt.getVolunteerIntegral());
            }
            clientLoginUser.setExtInfo(extInfo);
    ...
    public void updateUserInfo(ClientUserUpdateInfoParam clientUserUpdateInfoParam) {
        ...
        if(ObjectUtil.isNotEmpty(clientUserUpdateInfoParam.getAge())) {
            lambdaUpdateWrapper.set(ClientUser::getAge, clientUserUpdateInfoParam.getAge());
        }
        ...
        if(ObjectUtil.isNotEmpty(clientUserUpdateInfoParam.getNation())) {
            lambdaUpdateWrapper.set(ClientUser::getNation, clientUserUpdateInfoParam.getNation());
        }
        if(ObjectUtil.isNotEmpty(clientUserUpdateInfoParam.getNativePlace())) {
            lambdaUpdateWrapper.set(ClientUser::getNativePlace, clientUserUpdateInfoParam.getNativePlace());
        }
        if(ObjectUtil.isNotEmpty(clientUserUpdateInfoParam.getHomeAddress())) {
            lambdaUpdateWrapper.set(ClientUser::getHomeAddress, clientUserUpdateInfoParam.getHomeAddress());
        }
        if(ObjectUtil.isNotEmpty(clientUserUpdateInfoParam.getMailingAddress())) {
            lambdaUpdateWrapper.set(ClientUser::getMailingAddress, clientUserUpdateInfoParam.getMailingAddress());
        }
        if(ObjectUtil.isNotEmpty(clientUserUpdateInfoParam.getIdCardType())) {
            lambdaUpdateWrapper.set(ClientUser::getIdCardType, clientUserUpdateInfoParam.getIdCardType());
        }
        if(ObjectUtil.isNotEmpty(clientUserUpdateInfoParam.getIdCardNumber())) {
            lambdaUpdateWrapper.set(ClientUser::getIdCardNumber, clientUserUpdateInfoParam.getIdCardNumber());
        }
        if(ObjectUtil.isNotEmpty(clientUserUpdateInfoParam.getCultureLevel())) {
            lambdaUpdateWrapper.set(ClientUser::getCultureLevel, clientUserUpdateInfoParam.getCultureLevel());
        }
        if(ObjectUtil.isNotEmpty(clientUserUpdateInfoParam.getPoliticalOutlook())) {
            lambdaUpdateWrapper.set(ClientUser::getPoliticalOutlook, clientUserUpdateInfoParam.getPoliticalOutlook());
        }
        if(ObjectUtil.isNotEmpty(clientUserUpdateInfoParam.getCollege())) {
            lambdaUpdateWrapper.set(ClientUser::getCollege, clientUserUpdateInfoParam.getCollege());
        }
        if(ObjectUtil.isNotEmpty(clientUserUpdateInfoParam.getEducation())) {
            lambdaUpdateWrapper.set(ClientUser::getEducation, clientUserUpdateInfoParam.getEducation());
        }
        if(ObjectUtil.isNotEmpty(clientUserUpdateInfoParam.getEduLength())) {
            lambdaUpdateWrapper.set(ClientUser::getEduLength, clientUserUpdateInfoParam.getEduLength());
        }
        if(ObjectUtil.isNotEmpty(clientUserUpdateInfoParam.getDegree())) {
            lambdaUpdateWrapper.set(ClientUser::getDegree, clientUserUpdateInfoParam.getDegree());
        }
        if(ObjectUtil.isNotEmpty(clientUserUpdateInfoParam.getHomeTel())) {
            lambdaUpdateWrapper.set(ClientUser::getHomeTel, clientUserUpdateInfoParam.getHomeTel());
        }
        if(ObjectUtil.isNotEmpty(clientUserUpdateInfoParam.getOfficeTel())) {
            lambdaUpdateWrapper.set(ClientUser::getOfficeTel, clientUserUpdateInfoParam.getOfficeTel());
        }
        if(ObjectUtil.isNotEmpty(clientUserUpdateInfoParam.getEmergencyContact())) {
            lambdaUpdateWrapper.set(ClientUser::getEmergencyContact, clientUserUpdateInfoParam.getEmergencyContact());
        }
        if(ObjectUtil.isNotEmpty(clientUserUpdateInfoParam.getEmergencyPhone())) {
            lambdaUpdateWrapper.set(ClientUser::getEmergencyPhone, clientUserUpdateInfoParam.getEmergencyPhone());
        }
        if(ObjectUtil.isNotEmpty(clientUserUpdateInfoParam.getEmergencyAddress())) {
            lambdaUpdateWrapper.set(ClientUser::getEmergencyAddress, clientUserUpdateInfoParam.getEmergencyAddress());
        }
        // 更新指定字段
    ...
    @Transactional(rollbackFor = Exception.class)
    @Override
    public void doRegister(String account, String password,String type,String idCardType,String idCardNumber,String realName,String workUnit) {
        // 校验账号
        ...
        // 根据账号密码创建用户
        this.createUserWithAccountMini(account, password, type, idCardType, idCardNumber, realName, workUnit);
    }
    @Transactional
    protected void createUserWithAccountMini(String account, String password, String type, String idCardType, String idCardNumber, String realName, String workUnit) {
        ClientUserAddParam clientUserAddParam = new ClientUserAddParam();
        clientUserAddParam.setAccount(account);
        clientUserAddParam.setPhone(account);
        clientUserAddParam.setName(realName);
        clientUserAddParam.setType(type);
        clientUserAddParam.setIdCardType(idCardType);
        clientUserAddParam.setIdCardNumber(idCardNumber);
        clientUserAddParam.setWorkUnit(workUnit);
        clientUserAddParam.setPassword(password);
        clientUserAddParam.setGender(CommonGenderEnum.UNKNOWN.getValue());
        // 保存用户
        this.add(clientUserAddParam, ClientUserSourceFromTypeEnum.SYSTEM_REGISTER.getValue());
    ...
    @Override
    public List<ClientUser> getVolunteerListByParentId(String parentId) {
        if (StrUtil.isBlank(parentId)) {
            return List.of();
        }
        
        LambdaQueryWrapper<ClientUser> queryWrapper = new LambdaQueryWrapper<>();
        queryWrapper.eq(ClientUser::getParentId, parentId)
                   .eq(ClientUser::getType, "volunteer")  // 只查询志愿者类型
                   .eq(ClientUser::getUserStatus, "ENABLE")  // 只查询启用状态的用户
                   .orderByDesc(ClientUser::getCreateTime);
        
        return this.list(queryWrapper);
    }

    @Override
    public Page<ClientUser> volunteerAuditPage(ClientUserPageParam clientUserPageParam) {
        LambdaQueryWrapper<ClientUser> queryWrapper = new LambdaQueryWrapper<>();
        queryWrapper.eq(ClientUser::getType, "volunteer");
        
        // 添加搜索条件
        if (ObjectUtil.isNotEmpty(clientUserPageParam.getSearchKey())) {
            queryWrapper.and(wrapper -> wrapper
                .like(ClientUser::getName, clientUserPageParam.getSearchKey())
                .or()
                .like(ClientUser::getPhone, clientUserPageParam.getSearchKey())
                .or()
                .like(ClientUser::getAccount, clientUserPageParam.getSearchKey()));
        }
        
        // 添加审核状态筛选
        if (ObjectUtil.isNotEmpty(clientUserPageParam.getAuditStatus())) {
            queryWrapper.eq(ClientUser::getAuditStatus, clientUserPageParam.getAuditStatus());
        }
        
        queryWrapper.orderByDesc(ClientUser::getCreateTime);
        
        return this.page(CommonPageRequest.defaultPage(), queryWrapper);
    }

    @Override
    public ClientUser volunteerDetail(ClientUserIdParam clientUserIdParam) {
        ClientUser volunteer = this.queryEntity(clientUserIdParam.getId());
        if (ObjectUtil.isEmpty(volunteer) || !"volunteer".equals(volunteer.getType())) {
            throw new CommonException("小雷锋不存在");
        }
        
        // 查询监护人信息
        if (ObjectUtil.isNotEmpty(volunteer.getParentId())) {
            ClientUser parent = this.queryEntity(volunteer.getParentId());
            if (ObjectUtil.isNotEmpty(parent)) {
                // 在扩展信息中添加监护人信息
                JSONObject extInfo = new JSONObject();
                extInfo.set("parentName", parent.getName());
                extInfo.set("parentPhone", parent.getPhone());
                volunteer.setExtJson(extInfo.toString());
            }
        }
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void approveVolunteer(ClientUserIdParam clientUserIdParam) {
        ClientUser volunteer = this.queryEntity(clientUserIdParam.getId());
        if (ObjectUtil.isEmpty(volunteer)) {
            throw new CommonException("小雷锋不存在");
        }
        if (!"volunteer".equals(volunteer.getType())) {
            throw new CommonException("该用户不是小雷锋");
        }
        if (!"PENDING".equals(volunteer.getAuditStatus())) {
            throw new CommonException("该小雷锋已审核");
        }
        
        // 获取当前审核人
        String currentUserId = StpLoginUserUtil.getLoginUser().getId();
        
        // 更新审核状态
        LambdaUpdateWrapper<ClientUser> updateWrapper = new LambdaUpdateWrapper<>();
        updateWrapper.eq(ClientUser::getId, clientUserIdParam.getId())
                    .set(ClientUser::getAuditStatus, "APPROVED")
                    .set(ClientUser::getAuditTime, new Date())
                    .set(ClientUser::getAuditUserId, currentUserId)
                    .set(ClientUser::getAuditReason, null);
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void rejectVolunteer(ClientUserRejectParam rejectParam) {
        ClientUser volunteer = this.queryEntity(rejectParam.getId());
        if (ObjectUtil.isEmpty(volunteer)) {
            throw new CommonException("小雷锋不存在");
        }
        if (!"volunteer".equals(volunteer.getType())) {
        ...
        // 更新审核状态
        LambdaUpdateWrapper<ClientUser> updateWrapper = new LambdaUpdateWrapper<>();
        updateWrapper.eq(ClientUser::getId, rejectParam.getId())
                    .set(ClientUser::getAuditStatus, "REJECTED")
                    .set(ClientUser::getAuditTime, new Date())
                    .set(ClientUser::getAuditUserId, currentUserId)
                    .set(ClientUser::getAuditReason, rejectParam.getReason());
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void batchApproveVolunteer(ClientUserBatchAuditParam batchParam) {
        if (ObjectUtil.isEmpty(batchParam.getIds())) {
            throw new CommonException("请选择要审核的小雷锋");
        }
        ...
        // 批量更新审核状态
        LambdaUpdateWrapper<ClientUser> updateWrapper = new LambdaUpdateWrapper<>();
        updateWrapper.in(ClientUser::getId, batchParam.getIds())
                    .eq(ClientUser::getType, "volunteer")
                    .eq(ClientUser::getAuditStatus, "PENDING")
                    .set(ClientUser::getAuditStatus, "APPROVED")
                    .set(ClientUser::getAuditTime, new Date())
                    .set(ClientUser::getAuditUserId, currentUserId)
                    .set(ClientUser::getAuditReason, null);
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void batchRejectVolunteer(ClientUserBatchAuditParam batchParam) {
        if (ObjectUtil.isEmpty(batchParam.getIds())) {
            throw new CommonException("请选择要审核的小雷锋");
        }
        ...
        updateWrapper.in(ClientUser::getId, batchParam.getIds())
                    ...
                    .set(ClientUser::getAuditTime, new Date())
                    .set(ClientUser::getAuditUserId, currentUserId)
                    .set(ClientUser::getAuditReason, "批量审核拒绝");
