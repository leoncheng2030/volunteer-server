...
public class DevConfigApiProvider implements DevConfigApi {
    ...
    @Override
    public List<Map<String, String>> getBaseConfig() {
        List<DevConfig> devConfigs = devConfigService.sysBaseList();
        return devConfigs.stream().map(devConfig -> {
            Map<String, String> map = new HashMap<>(2);
            map.put("key", devConfig.getConfigKey());
            map.put("value", devConfig.getConfigValue());
            return map;
        }).toList();
    }
}
